...
double Pokemon::getBaseStat(Stat x) const {
    try {
        return std::round(this->baseStats.at(static_cast<std::size_t>(x)) * this->getStat(x));
    }
    catch (const std::exception &e) {
...
bool Pokemon::isFasterThan(const Pokemon &pokemon) const {
    ...
    return this->baseStats.at(static_cast<std::size_t>(Stat::SPEED)) * speedMod1 >
           pokemon.baseStats.at(static_cast<std::size_t>(Stat::SPEED)) * speedMod2;
}
