...
void TextBox::render() {
    TextureManager::getInstance().drawRect(this->rect, { 255, 255, 255 }, { 0, 0, 0 }, border);
    ...
    if (this->lettersPrinted <= this->messages.front().length()) {
        ...
        // recreate the text
        SDL_Surface *temp = TTF_RenderUTF8_Blended_Wrapped(TextBox::font, buffer.c_str(), { 0, 0, 0 }, this->rect.w);
    ...
    TextureManager::getInstance().draw(this->text,
                                       { this->textPos.first, this->textPos.second, textWidth, textHeight });
}
